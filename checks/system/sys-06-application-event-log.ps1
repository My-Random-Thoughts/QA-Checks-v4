...
Function sys-06-application-event-log
{
    $result        = newResult
    $result.server = $env:ComputerName
    $result.name   = $script:lang['Name']
    $result.check  = 'sys-06-application-event-log'
        ...
        [xml]   $xml        = @"
            <QueryList>
                <Query Id="0" Path="Application">
                    <Select Path="Application">*[System[(Level=1 or Level=2 $incWarn) and TimeCreated[timediff(@SystemTime) &lt;= $timeOffSet]]]
                    </Select>
                </Query>
            </QueryList>
"@
        ...
        [psobject]$gITMp = (Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\EventLog\Application' -Name ('MaxSize', 'Retention', 'AutoBackupLogFiles') -ErrorAction SilentlyContinue)
