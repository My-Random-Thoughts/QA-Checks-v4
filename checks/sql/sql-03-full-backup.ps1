<#
    DESCRIPTION:
        Checks that all full backups have completed successfully.
    ...
    RESULTS:
        PASS:
            There are no missing full backups
        WARNING:
        FAIL:
            There are one or more missing full backups
        MANUAL:
        NA:
...
Function sql-03-full-backup
{
    $result        = newResult
    $result.server = $env:ComputerName
    $result.name   = $script:lang['Name']
    $result.check  = 'sql-03-full-backup'
...
DECLARE MISSING_FULL_BACKUPS CURSOR FOR
...
OPEN MISSING_FULL_BACKUPS
    FETCH NEXT FROM MISSING_FULL_BACKUPS INTO @V_MISSING_BCK
    SET @V_ROWCOUNT = 0
    ...
    END
CLOSE MISSING_FULL_BACKUPS
DEALLOCATE MISSING_FULL_BACKUPS
